...
function getEssayData() {
	...
	.done(function(data) {
		$('#title').val(data.essayHeader);
		$('#country').val(data.country);
		editor.contentWindow.document.body.innerHTML += data.context;
		titlepic = data.essayPictureURL;
	})
	.fail(function() {
		console.log("error");
	})
...
$('#submitPic').click(function(event) {
	 $('.submitPic').click();
	 $('.submitPic').change(function(event) {
 		// 上传图片模块
		$.ajaxFileUpload({
...
$('#send').click(function(event) {
	...
	if($('#title').val() == '' || $('#country').val() == '' || $('#input-area').val() == ''){
		alert("请将表单填写完整");
	}else{
		if($("#headerpic").val() == ''){
			addEassay(titlepic);	      //用户没有更改标题图片，直接修改文章
		}else{
			submitPic();
		}
	}
...
function addEassay(pic) {
	$.ajax({
		// url: 'data/detail.json',
		url: 'http://localhost:8080/travel/essay/update',
		type: 'post',
		dataType: 'json',
		contentType:'application/json',
		data: JSON.stringify({
